...
$task_sql = 
  'SELECT task.task_id AS id, task.ttid, d.name, inst.value AS instances, attr.value AS features ' . 
  'FROM task, task_tag `t`, task_inputs `i`, dataset d ' . 
  ...
  'AND i.value = d.did ' .
  $where_tag;
$this->task_columns = array( 'id', 'ttid', 'name', 'instances', 'features' );
