    ...
    $.ajax({
        ...
        if(resultdata.getElementsByTagName('downvote').length>0){
            ...
            for(var i=0; i<dvotes.length; i++){
                ...
                $('#issues_content').append('<td><a id="downvotebutton-'+id+'" class="loginfirst btn btn-link" onclick="doDownvote('+id+')" title="Click to agree"> <i id="downvoteicon-'+id+'" class="fa fa-thumbs-o-down"/></a></td>');
                $('#issues_content').append('</tr>');
            }            
            if(reason_id!=-1){
