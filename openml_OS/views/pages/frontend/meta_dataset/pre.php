...
$sql = 'SELECT `m`.`id`, `m`.`request_date`, ' .
       'IF(CHAR_LENGTH(`m`.`datasets`)>20, CONCAT( SUBSTRING(`m`.`datasets`, 1, 15), "..."), `m`.`datasets`) AS `datasets`, ' .
       'IF(CHAR_LENGTH(`m`.`tasks`)>20, CONCAT( SUBSTRING(`m`.`tasks`, 1, 15), "..."), `m`.`tasks`) AS `tasks`, ' . 
       'IF(CHAR_LENGTH(`m`.`flows`)>20, CONCAT( SUBSTRING(`m`.`flows`, 1, 15), "..."), `m`.`flows`) AS `flows`, ' . 
       'IF(CHAR_LENGTH(`m`.`setups`)>20, CONCAT( SUBSTRING(`m`.`setups`, 1, 15), "..."), `m`.`setups`) AS `setups`, ' . 
       '`m`.`functions`, ' .
